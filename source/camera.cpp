...
void cameraUpdate(Camera &cam, u32 kHeld)
{
    ...
    // derecha
    C3D_FVec right = FVec4_New(cosf(cam.yaw), 0.0f, sinf(cam.yaw), 0.0f);
...
void cameraViewMatrix(const Camera &cam, C3D_Mtx *out)
{
    ...
    C3D_FVec up = FVec4_New(0.0f, 1.0f, 0.0f, 0.0f); // "arriba" del mundo siempre es +Y
